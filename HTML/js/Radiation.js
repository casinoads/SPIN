...
function renderShareImg(gift_id, object) {
  ...
  $.ajax({
    ...
    style="background-image: url('./rotation/assets/IMAGES/ปั่นปุ๊ป-ลุ้นโชค.jpeg');background-size: cover;width:1200px;height:630px;font-family: 'Kanit'">
    ...
    <div>
        ...
        ยินดีด้วยกับบัตรขูด ${getGiftPhone(
        data.data.gift_id
      )}</span>
          </div>
          ...
          <div>
              <span style="font-size: 23pt;color: #fff;line-height: 28pt;">
              หมุนต่อไปเพื่อรับของขวัญมากขึ้น</span>
          </div>
      </div>
      <img src="./rotation/assets/Gift/${getGift(
        data.data.gift_id
      )}.png" width="170px"
...
function showShareDialog(gift_id_specific) {
  ...
  html2canvas(element, {
    ...
    onclone: function (clonedDoc) {
      $("#append-share").css("font-family", "Kanit");
...
(function (d, s, id) {
  ...
  js.id = id;
  js.src =
    "//connect.facebook.net/en_US/sdk.js#xfbml=1&version=v10.0&appId=763677864338153&autoLogAppEvents=1";
  ...
  FB.ui(
    {
      method: "feed",
      display: "touch",
      link: `https://ufapro888s.info/event/share?id=${gift_id_specific}`,
    },
    function (t) {
...
function initSpinImage() {
  ...
  if (wWidth >= 481) {
    ...
    loadedImg.src = "./rotation/assets/IMAGES/wheel.png";
    $("#nhap-nhay").css(
      "background-image",
      "url(" + "./rotation/assets/IMAGES/spinwheel-cb-outline.gif" + ")"
    );
  } else {
    loadedImg.src = "./rotation/assets/IMAGES/wheel-mob.png";
    $("#nhap-nhay").css(
      "background-image",
      "url(" + "/rotation/assets/IMAGES/spinwheel-cb-outline.gif" + ")"
    );
    ...
    success: function (data) {
      ...
      if (data.data.voucher) {
        ...
        if (data.data.name == null) {
          ...
          enableFills(0);
          $("#modal-card-tutor-bottom, #modal-card-tutor-top").hide();
          const imgHtml = `<img src="/images/ปั่นปุ๊ป-ลุ้นโชค/${id}.jpeg" width="82%" style="border-radius:5px"><div class="p-1"></div>`;
